document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("toonOverzicht").addEventListener("click", function () {
    ...
    firebase.database().ref("uitgaven").once("value", snapshot => {
      ...
      header.innerHTML = "<th>Groep</th><th>Totaal (€)</th><th>Leden</th><th>Max/lid (€)</th><th>Status</th>";
      ...
      alleGroepen.forEach(groep => {
        ...
        const totaal = totaalPerGroep[groep];
        const leden = parseInt(document.getElementById(`leden-${groep}`).value) || 0;
        const maxPerLid = parseFloat(document.getElementById(`maxbedrag-${groep}`).value) || 0;
        const maxTotaal = leden * maxPerLid;

        rij.insertCell(0).textContent = groep;
        rij.insertCell(1).textContent = `€${totaal.toFixed(2)}`;
        rij.insertCell(2).textContent = leden;
        rij.insertCell(3).textContent = `€${maxPerLid.toFixed(2)}`;

        const statusCel = rij.insertCell(4);
        if (leden > 0 && maxPerLid > 0) {
          if (totaal > maxTotaal) {
            statusCel.textContent = "❌ Over budget";
            statusCel.style.color = "red";
          } else {
            statusCel.textContent = "✅ Binnen budget";
            statusCel.style.color = "green";
          }
        } else {
          statusCel.textContent
